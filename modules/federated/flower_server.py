...
Sets up and manages Flower server for federated learning coordination.
"""
...
from .utils import retry, safe_execute, GracefulDegradation, generate_auth_token, validate_client_token
...
class FLServerManager:
    """
    Manages Flower server for federated learning.
    """
    ...
    def __init__(
        ...
        results_file: str = "data/fl_results.json"
    ):
        """
    ...
    def create_strategy(self) -> FedAvg:
        """
        Create FedAvg strategy with configuration.
        ...
        def fit_metrics_aggregation_fn(results: List[Tuple[int, FitRes]]) -> Dict:
            ...
            wers = [res.metrics.get("wer", 1.0) for _, res in results]
            ...
            if total_samples > 0:
                ...
                avg_wer = sum(w * n for w, n in zip(wers, num_samples)) / total_samples
            else:
        ...
        strategy = FedAvg(
            ...
            fraction_evaluate=self.config.fraction_fit,  # Evaluate same fraction
    ...
    def start_server(
        ...
        if self.client_fn:
            ...
            self.server = Server(
                client_manager=None,  # Will use client_fn
                strategy=strategy
            )
        else:
    ...
    def stop_server(self):
        ...
        self.is_running = False
        # Note: Flower doesn't have explicit stop, but we can mark as stopped
    ...
    def run_federated_learning(
        ...
        # For now, we'll simulate the rounds
        if self.client_fn:
    ...
    def add_monitoring_callback(self, callback: Callable[[Dict], None]):
        ...
        Args:
            callback: Function that takes round metrics dict
        """
    ...
    def get_metrics_history(self) -> Dict:
        ...
        return {
            ...
            "loss_history": [r.get("metrics", {}).get("loss", 0.0) for r in self.round_metrics],
            "wer_history": [r.get("metrics", {}).get("wer", 1.0) for r in self.round_metrics],
